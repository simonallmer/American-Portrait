...
(function () {
  ...
  if (hash.indexOf('#play-') === 0) {
    var token = hash.replace('#play-', ''), idx = -1;
    if (/^\d+$/.test(token)) idx = parseInt(token, 10);
    else {
      for (var puzzleIndex = 0; puzzleIndex < CL.puzzles.length; puzzleIndex++) {
        if (CL.puzzles[puzzleIndex].id === token) { idx = puzzleIndex; break; }
      }
    }
    if (idx >= 0 && idx < CL.puzzles.length) {
      go("play", idx);
    } else {
      go("menu");
    }
